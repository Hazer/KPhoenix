...
import io.vithor.Presence.PresenceMap
...
//        constructor(map: Map<String, Any>? = null) : this(map?.mapValues { it as PresenceMap } ?: mutableMapOf())
    ...
    init {
        this.channel.on(events.state) { msg ->
            val newState = presenceStateOf(msg?.payload)
        ...
        this.channel.on(events.diff) { msg ->
            val diff: Diff = msg?.payload?.mapValues { it as PresenceState } ?: mapOf()
            val (onJoin, onLeave, onSync) = this.caller
...
//        }
    ...
    val List<Presence.Event>.state: String
        get() {
            return find { it.type == Presence.PresenceEventType.State }!!.value
        }

    val List<Presence.Event>.diff: String
        get() {
            return find { it.type == Presence.PresenceEventType.Diff }!!.value
        }
}

fun presenceStateOf(map: Map<String, Any>? = null) =
    Presence.PresenceState(map?.mapValues { it as PresenceMap } ?: mutableMapOf())
