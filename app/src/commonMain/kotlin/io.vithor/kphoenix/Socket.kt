...
import io.vithor.kphoenix.facades.*
...
//    open fun protocol(): String = if (location.protocol.matches("/^https/".toRegex())) "wss" else "ws"
    ...
    open fun endPointURL(): String {
        return location
            .with(this.params, mapOf("vsn" to VSN, "phx-client-name" to Platform.name))
            .also {
                log("endpoint", it)
            }
    }
    ...
    open fun sendHeartbeat() {
        if (this.pendingHeartbeatRef != null) {
            this.pendingHeartbeatRef = null
            if (this.hasLogger())
                this.log("path", "heartbeat timeout. Attempting to re-establish connection")
        ...
        pendingHeartbeatRef.also { pendingHeartbeatRef ->
            this.push(
                Message(
                    topic = "phoenix",
                    event = "heartbeat",
